...
def main():
    ...
    deployed, deploy_port = _deploy_via_pbip(bim_path, output_dir, workbook_name, data_dir)
...
def _create_pbip_project(bim_path, output_dir, workbook_name):
...
def _deploy_via_pbip(bim_path, output_dir, workbook_name, data_dir):
    """Deploy via PBIP project: create project, open in PBI, run TE2 scripts."""

    # --- Create PBIP project ---
    Status.info("Creating Power BI project (PBIP)...")
    pbir_path = _create_pbip_project(bim_path, output_dir, workbook_name)
    ...
    # The .bim already contains everything (tables, columns, calc columns,
    # measures, relationships, display folders). No TE2 scripts needed.
    time.sleep(3)
